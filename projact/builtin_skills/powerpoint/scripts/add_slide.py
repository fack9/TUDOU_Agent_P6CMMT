...
def get_next_slide_number(slides_dir: Path) -> int:
    existing = [int(m.group(1)) for f in slides_dir.glob('slide*.xml') if (m := re.match('slide(\\d+)\\.xml', f.name))]
...
def duplicate_slide(unpacked_dir: Path, source: str) -> None:
    ...
    if source_rels.exists():
        ...
        rels_content = re.sub('\\s*<Relationship[^>]*Type="[^"]*notesSlide"[^>]*/>\\s*', '\n', rels_content)
...
def _add_to_presentation_rels(unpacked_dir: Path, dest: str) -> str:
    ...
    rids = [int(m) for m in re.findall('Id="rId(\\d+)"', pres_rels)]
...
def _get_next_slide_id(unpacked_dir: Path) -> int:
    ...
    slide_ids = [int(m) for m in re.findall('<p:sldId[^>]*id="(\\d+)"', pres_content)]
