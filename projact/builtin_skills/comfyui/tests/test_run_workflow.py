...
from run_workflow import ComfyRunner, download_outputs, inject_params, parse_input_image_arg
...
class TestInjectParams:
    ...
    def test_basic_injection(self, sd15_workflow):
        ...
        wf, warnings = inject_params(sd15_workflow, schema, {'prompt': 'new prompt', 'seed': 999, 'steps': 25})
    ...
    def test_refuses_to_overwrite_link(self):
        wf = {'1': {'class_type': 'CheckpointLoaderSimple', 'inputs': {'ckpt_name': 'x'}}, '5': {'class_type': 'EmptyLatentImage', 'inputs': {'width': 512, 'height': 512, 'batch_size': 1}}, '6': {'class_type': 'CLIPTextEncode', 'inputs': {'text': ['3', 0], 'clip': ['1', 1]}}, '3': {'class_type': 'KSampler', 'inputs': {'seed': 1, 'steps': 20, 'cfg': 7.5, 'sampler_name': 'euler', 'scheduler': 'normal', 'denoise': 1.0, 'model': ['1', 0], 'positive': ['6', 0], 'negative': ['6', 0], 'latent_image': ['5', 0]}}, '9': {'class_type': 'SaveImage', 'inputs': {'filename_prefix': 'x', 'images': ['3', 0]}}}
        schema = {'parameters': {'prompt': {'node_id': '6', 'field': 'text', 'type': 'string', 'value': ''}}}
    ...
    def test_handles_videos_plural(self, tmp_path, monkeypatch):
        ...
        class FakeRunner:
            ...
            def download_output(self, *, filename, subfolder, file_type, output_dir, preserve_subfolder, overwrite):
                ...
                p.write_bytes(b'x')
                return p
        outputs = {'9': {'images': [{'filename': 'img1.png', 'subfolder': '', 'type': 'output'}]}, '10': {'videos': [{'filename': 'vid1.mp4', 'subfolder': '', 'type': 'output'}]}, '11': {'gifs': [{'filename': 'anim1.gif', 'subfolder': '', 'type': 'output'}]}}
    ...
    def test_handles_video_singular_cloud(self, tmp_path):
        ...
        class FakeRunner:
            ...
            def download_output(self, *, filename, subfolder, file_type, output_dir, preserve_subfolder, overwrite):
                ...
                p.write_bytes(b'x')
                return p
        outputs = {'10': {'video': [{'filename': 'cloud.mp4', 'subfolder': '', 'type': 'output'}]}}
        result = download_outputs(FakeRunner(), outputs, tmp_path)
        assert len(result) == 1
    ...
    def test_preserves_subfolder(self, tmp_path):
        ...
        class FakeRunner:
            ...
            def download_output(self, *, filename, subfolder, file_type, output_dir, preserve_subfolder, overwrite):
                ...
                p.write_bytes(b'x')
                return p
        outputs = {'9': {'images': [{'filename': 'img.png', 'subfolder': 'myrun', 'type': 'output'}, {'filename': 'img.png', 'subfolder': 'otherrun', 'type': 'output'}]}}
