...
class TestIsLink:
    ...
    def test_non_link(self):
        ...
        assert is_link(['3']) is False
        assert is_link(['3', '0']) is False
        assert is_link([3, 0]) is False
...
class TestIterEmbeddingRefs:
    ...
    def test_finds_in_clip_text_encode(self):
        wf = {'1': {'class_type': 'CLIPTextEncode', 'inputs': {'text': 'embedding:foo, embedding:bar:0.5', 'clip': ['2', 0]}}, '2': {'class_type': 'CheckpointLoaderSimple', 'inputs': {'ckpt_name': 'x'}}}
        refs = list(iter_embedding_refs(wf))
        names = [name for _, name in refs]
        assert names == ['foo', 'bar']

    def test_ignores_non_prompt_fields(self):
        wf = {'1': {'class_type': 'CheckpointLoaderSimple', 'inputs': {'ckpt_name': 'embedding:foo.safetensors'}}}
        refs = list(iter_embedding_refs(wf))
        assert refs == []
...
class TestParseModelList:
    ...
    def test_cloud_format_dicts(self):
        result = parse_model_list([{'name': 'a.safetensors', 'pathIndex': 0}, {'name': 'b.safetensors', 'pathIndex': 1}])
        assert result == {'a.safetensors', 'b.safetensors'}
    ...
    def test_mixed_format(self):
        result = parse_model_list(['string-form.safetensors', {'name': 'dict-form.safetensors'}])
...
class TestRedirectHeaderStripping:
    ...
    def test_strips_x_api_key_cross_host(self):
        ...
        prep.prepare(method='GET', url='https://other.example.com/file', headers={'X-API-Key': 'leak', 'Authorization': 'Bearer x'})
    ...
    def test_strips_cookie_cross_host(self):
        ...
        s = self._build_session()
        prep = requests.PreparedRequest()
        prep.prepare(method='GET', url='https://other.example.com/x', headers={'Cookie': 'session=secret'})
        resp = requests.Response()
        orig = requests.PreparedRequest()
