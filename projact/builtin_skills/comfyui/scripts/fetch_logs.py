...
from _common import DEFAULT_LOCAL_HOST, ENV_API_KEY, emit_json, http_get, is_cloud_host, resolve_api_key, resolve_url
...
def fetch_history_entry(host: str, headers: dict, prompt_id: str, *, is_cloud: bool) -> dict:
    ...
    if not isinstance(data, dict) or prompt_id not in data:
        return {'ok': False, 'reason': 'prompt_id not found in history', 'history_keys': list(data.keys())[:5] if isinstance(data, dict) else []}
...
def main(argv: list[str] | None=None) -> int:
    ...
    p.add_argument('--raw', action='store_true', help='Print the full history entry instead of the digest')
    p.add_argument('--tail-queue', action='store_true', help='Show currently running/pending jobs instead')
