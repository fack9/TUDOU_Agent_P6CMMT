...
OK_MAC_RAM_GB = 32
MIN_FREE_DISK_GB = 25
_COMFY_CLI_FLAG = {'nvidia': '--nvidia', 'amd': '--amd', 'apple-silicon': '--m-series', 'intel': None, 'comfy-cloud': None, 'cpu': '--cpu'}
...
def detect_nvidia() -> dict | None:
    ...
    for line in out.strip().splitlines():
        ...
        gpus.append({'vendor': 'nvidia', 'index': idx, 'name': name, 'vram_gb': round(vram_mb / 1024, 1), 'driver': driver})
    if not gpus:
        return None
...
def detect_rocm() -> dict | None:
        ...
        try:
            ...
            for card_id, info in data.items():
                if not card_id.startswith('card'):
                    continue
                name = info.get('Card series') or info.get('Card model') or info.get('Marketing Name') or 'AMD GPU'
                vram_b = info.get('VRAM Total Memory (B)') or info.get('vram_total_memory_b') or 0
                try:
                    vram_b = int(vram_b)
                except (ValueError, TypeError):
                    vram_b = 0
                cards.append({'vendor': 'amd', 'name': str(name).strip(), 'vram_gb': round(vram_b / 1024 ** 3, 1), 'driver': 'rocm'})
            if cards:
    ...
    if not out.strip():
        return None
    name_m = re.search('Card (?:series|model|Marketing Name):\\s*(.+)', out)
    vram_m = re.search('VRAM Total Memory \\(B\\):\\s*(\\d+)', out)
    vram_gb = round(int(vram_m.group(1)) / 1024 ** 3, 1) if vram_m else 0.0
    return {'vendor': 'amd', 'name': name_m.group(1).strip() if name_m else 'AMD GPU', 'vram_gb': vram_gb, 'driver': 'rocm'}
...
def detect_apple_silicon() -> dict | None:
    ...
    for v in ('Ultra', 'Max', 'Pro'):
        if v in chip:
            variant = v
            break
    return {'vendor': 'apple', 'name': chip or 'Apple Silicon', 'generation': generation, 'variant': variant, 'unified_memory_gb': ram_gb}
...
def detect_intel_arc() -> dict | None:
    ...
    if platform.system() == 'Windows' and shutil.which('powershell'):
        out = _run(['powershell', '-NoProfile', 'Get-CimInstance Win32_VideoController | Select-Object Name | Format-List'])
...
def total_system_ram_gb() -> float:
    ...
    if sysname == 'Windows':
        if shutil.which('powershell'):
            out = _run(['powershell', '-NoProfile', '(Get-CimInstance Win32_ComputerSystem).TotalPhysicalMemory'])
            m = re.search('(\\d{8,})', out)
            if m:
...
def check_pytorch_cuda() -> dict | None:
    ...
    info: dict[str, Any] = {'available': True, 'torch_version': torch.__version__}
    try:
...
def classify(gpu: dict | None, ram_gb: float, free_disk_gb: float, *, wsl: bool, rosetta: bool) -> tuple[str, str, list[str]]:
    notes: list[str] = []
    if rosetta:
        notes.append('Detected Python running under Rosetta on Apple Silicon. ComfyUI MPS support requires native ARM64 Python — install via `brew install python` or arm64 Miniforge, then re-run.')
    ...
    if free_disk_gb and free_disk_gb < MIN_FREE_DISK_GB:
        notes.append(f'Free disk space ({free_disk_gb} GB) is below the {MIN_FREE_DISK_GB} GB recommended minimum. ComfyUI core (~5 GB) plus one SDXL model (~6.5 GB) needs space; Flux Dev needs ~24 GB.')
    if ram_gb and ram_gb < 8 and gpu and (gpu.get('vendor') != 'apple'):
        notes.append(f'System RAM ({ram_gb} GB) is low. ComfyUI swaps model weights through host RAM; <8 GB causes severe slowdowns. 16+ GB recommended.')
    if gpu is None:
        notes.append('No supported accelerator found (NVIDIA CUDA / AMD ROCm / Apple Silicon / Intel Arc).')
        notes.append('CPU-only ComfyUI works but is unusably slow for modern models — use Comfy Cloud.')
        return ('cloud', 'comfy-cloud', notes)
    if gpu['vendor'] == 'apple':
        ...
        if mem < MIN_MAC_RAM_GB:
            notes.append(f'{gen_str} with {mem} GB unified memory — below the {MIN_MAC_RAM_GB} GB practical minimum.')
            notes.append('SD1.5 may work; SDXL/Flux will swap or OOM. Recommend Comfy Cloud.')
            return ('cloud', 'comfy-cloud', notes)
        if mem < OK_MAC_RAM_GB:
            notes.append(f'{gen_str} with {mem} GB — SDXL works but slow. Flux/video likely too tight.')
    ...
    if vram < MIN_VRAM_GB_USABLE:
        notes.append(f'{name} has only {vram} GB VRAM — below the {MIN_VRAM_GB_USABLE} GB practical minimum.')
...
def build_report(*, check_pytorch: bool=False) -> dict:
    ...
    if gpu is None and sysname == 'Darwin' and (arch != 'arm64') and (not rosetta):
        notes = ['Intel Mac detected — no MPS backend available.', 'ComfyUI will fall back to CPU which is unusably slow. Use Comfy Cloud.']
        report = {'os': sysname, 'arch': arch, 'system_ram_gb': ram_gb, 'free_disk_gb': free_disk_gb, 'wsl': False, 'rosetta': False, 'gpu': None, 'verdict': 'cloud', 'recommended_install_path': 'comfy-cloud', 'comfy_cli_flag': None, 'notes': notes, 'install_urls': _install_urls()}
        if check_pytorch:
            report['pytorch'] = check_pytorch_cuda()
        return report
    verdict, install_path, notes = classify(gpu, ram_gb, free_disk_gb, wsl=wsl, rosetta=rosetta)
    report = {'os': sysname, 'arch': arch, 'system_ram_gb': ram_gb, 'free_disk_gb': free_disk_gb, 'wsl': wsl, 'rosetta': rosetta, 'gpu': gpu, 'verdict': verdict, 'recommended_install_path': install_path, 'comfy_cli_flag': _COMFY_CLI_FLAG.get(install_path), 'notes': notes, 'install_urls': _install_urls()}
    if check_pytorch:
        report['pytorch'] = check_pytorch_cuda()
    return report

def _install_urls() -> dict:
    return {'desktop': 'https://docs.comfy.org/installation/desktop', 'manual': 'https://docs.comfy.org/installation/manual_install', 'comfy_cli': 'https://docs.comfy.org/comfy-cli/getting-started', 'cloud': 'https://platform.comfy.org'}
...
def main(argv: list[str] | None=None) -> int:
    ...
    p.add_argument('--check-pytorch', action='store_true', help='Also probe `torch` for CUDA/MPS availability (slower)')
