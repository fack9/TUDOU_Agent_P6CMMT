...
from _common import DEFAULT_LOCAL_HOST, ENV_API_KEY, coerce_seed, emit_json, http_get, http_post, http_request, is_cloud_host, is_link, log, looks_like_video_workflow, media_type_from_filename, new_client_id, resolve_api_key, resolve_url, safe_path_join, unwrap_workflow
...
class ComfyRunner:
    ...
    def __init__(self, host: str=DEFAULT_LOCAL_HOST, api_key: str | None=None, client_id: str | None=None, partner_key: str | None=None):
    ...
    def upload_image(self, path: Path, *, image_type: str='input', overwrite: bool=True, endpoint: str='/upload/image', extra_form: dict | None=None) -> dict:
        if not path.exists():
        ...
        with path.open('rb') as fh:
            ...
            r = http_request('POST', self._url(endpoint), headers=self.headers, files=files, form=form, timeout=300, retries=2)
        if r.status != 200:
            raise WorkflowRunError('upload_failed', f'Upload of {path.name} failed: HTTP {r.status}', body=r.text()[:500])
        try:
            return r.json()
        except Exception:
    ...
    def upload_mask(self, path: Path, original_ref: dict) -> dict:
        return self.upload_image(path, endpoint='/upload/mask', extra_form={'subfolder': 'clipspace', 'original_ref': json.dumps(original_ref)})
    ...
    def poll_status(self, prompt_id: str, *, timeout: float=300.0, initial_interval: float=1.5, max_interval: float=8.0) -> dict:
        start = time.time()
        interval = initial_interval
        while time.time() - start < timeout:
            if self.is_cloud:
                r = http_get(self._url(f'/job/{prompt_id}/status'), headers=self.headers, retries=2, timeout=30)
                if r.status == 200:
                    try:
                        data = r.json()
                    except Exception:
                ...
                r = http_get(self._url(f'/history/{prompt_id}'), headers=self.headers, retries=2, timeout=30)
                if r.status == 200:
                    try:
                        data = r.json() or {}
                    except Exception:
    ...
    def monitor_ws(self, prompt_id: str, *, timeout: float=300.0, on_progress: Any=None) -> dict:
        try:
            import websocket
        except ImportError:
            ...
            while time.time() < deadline:
                ...
                if mtype == 'progress':
                    if callable(on_progress):
                        on_progress({'type': 'progress', 'value': mdata.get('value'), 'max': mdata.get('max'), 'node': mdata.get('node')})
    ...
    def download_output(self, *, filename: str, subfolder: str, file_type: str, output_dir: Path, preserve_subfolder: bool=True, overwrite: bool=False) -> Path:
        ...
        r = http_request('GET', url, headers=self.headers, timeout=600, retries=3, follow_redirects=True, stream=True, sink=out_path)
        if r.status != 200:
            try:
                if out_path.exists():
                    out_path.unlink()
                ...
                pass
            raise WorkflowRunError('download_failed', f'Download of {filename} failed: HTTP {r.status}', url=url)
        return out_path

    def cancel(self, prompt_id: str | None=None) -> bool:
        if prompt_id:
            r = http_post(self._url('/queue'), headers=self.headers, json_body={'delete': [prompt_id]}, retries=1)
            return r.status == 200
...
def inject_params(workflow: dict, schema: dict, args: dict, *, randomize_seed_if_unset: bool=False) -> tuple[dict, list[str]]:
    ...
    for name, value in args.items():
        ...
        if is_link(cur):
            warnings.append(f"parameter '{name}' targets {nid}.{field} which is currently a link; refusing to overwrite (set the schema to point at the source node instead)")
            continue
        node['inputs'][field] = value
    return (wf, warnings)

def download_outputs(runner: ComfyRunner, outputs: dict, output_dir: Path, *, preserve_subfolder: bool=True, overwrite: bool=False) -> list[dict]:
    ...
    for node_id, node_output in (outputs or {}).items():
        ...
        for key in OUTPUT_KEYS:
            ...
            for fi in entries:
                ...
                try:
                    out_path = runner.download_output(filename=filename, subfolder=subfolder, file_type=file_type, output_dir=output_dir, preserve_subfolder=preserve_subfolder, overwrite=overwrite)
                    downloaded.append({'file': str(out_path), 'node_id': node_id, 'type': media_type_from_filename(filename), 'filename': filename, 'subfolder': subfolder, 'source_type': file_type})
                except Exception as e:
...
def main(argv: list[str] | None=None) -> int:
    p = argparse.ArgumentParser(description='Run a ComfyUI workflow with parameter injection.', formatter_class=argparse.RawDescriptionHelpFormatter)
    p.add_argument('--workflow', required=True, help='Path to workflow API JSON file')
    p.add_argument('--args', default='{}', help='JSON parameters to inject (or `@/path/to/args.json`)')
    ...
    p.add_argument('--partner-key', help='Partner-node API key (extra_data.api_key_comfy_org). Required for Flux Pro / Ideogram / etc. Defaults to --api-key if not set.')
    p.add_argument('--output-dir', default='./outputs', help='Directory to save outputs')
    p.add_argument('--timeout', type=int, default=0, help='Max seconds to wait (0=auto: 300 / 900 for video workflows)')
    p.add_argument('--input-image', action='append', default=[], help='Upload local image before running. Format: `name=path` or `path`. The `name` becomes the value injected into the matching schema parameter.')
    p.add_argument('--randomize-seed', action='store_true', help="If schema has a 'seed' parameter and --args didn't set one, randomize it")
    p.add_argument('--ws', action='store_true', help='Use WebSocket for real-time progress (requires `websocket-client`)')
    p.add_argument('--no-download', action='store_true', help='Skip downloading outputs')
    p.add_argument('--flat-output', action='store_true', help="Don't preserve server-side subfolder structure when saving outputs")
    p.add_argument('--overwrite', action='store_true', help='Overwrite existing files instead of appending _1, _2, ...')
    p.add_argument('--submit-only', action='store_true', help='Submit and return prompt_id without waiting')
    p.add_argument('--client-id', help='Override generated client_id (UUID)')
    p.add_argument('--use-partner-key-as-auth', action='store_true', help="(Compat) Use --partner-key value as cloud X-API-Key. Don't use unless you know why.")
    ...
    if args.use_partner_key_as_auth and (not api_key) and partner_key:
        api_key = partner_key
    runner = ComfyRunner(host=args.host, api_key=api_key, partner_key=partner_key, client_id=args.client_id)
    ok, info = runner.check_server()
    if not ok:
        emit_json({'error': f'Cannot reach server at {args.host}', 'details': info, 'hint': f'Check `comfy launch --background` is running for local, or set ${ENV_API_KEY} for cloud.'})
        return 1
    ...
    workflow, inj_warnings = inject_params(workflow, schema, user_args, randomize_seed_if_unset=args.randomize_seed)
    warnings = upload_warnings + inj_warnings
    for w in warnings:
        log(f'WARN: {w}')
    submit_resp = runner.submit(workflow)
    if '_http_error' in submit_resp:
        emit_json({'error': 'Submission HTTP error', 'http_status': submit_resp['_http_error'], 'body': submit_resp.get('body')})
        return 1
    if isinstance(submit_resp.get('error'), dict):
        emit_json({'error': 'Workflow validation failed', 'details': submit_resp['error'], 'node_errors': submit_resp.get('node_errors')})
        return 1
    ...
    try:
        ...
        emit_json({'status': 'interrupted', 'prompt_id': prompt_id, 'note': 'Ctrl+C received; sent cancellation to server.'})
        return 130
    if wait_result['status'] == 'timeout':
        emit_json({'status': 'timeout', 'prompt_id': prompt_id, 'elapsed': wait_result.get('elapsed'), 'hint': 'Re-run with larger --timeout, or use --submit-only and check later.'})
        return 1
    ...
    if args.no_download:
        emit_json({'status': 'success', 'prompt_id': prompt_id, 'outputs': outputs, 'warnings': warnings})
        return 0
    downloaded = download_outputs(runner, outputs, Path(args.output_dir).expanduser(), preserve_subfolder=not args.flat_output, overwrite=args.overwrite)
    emit_json({'status': 'success', 'prompt_id': prompt_id, 'outputs': downloaded, 'warnings': warnings})
    return 0
