...
from _common import DEFAULT_LOCAL_HOST, ENV_API_KEY, emit_json, http_get, parse_model_list, resolve_api_key, resolve_url, unwrap_workflow

def comfy_cli_status() -> dict:
    if shutil.which('comfy'):
        return {'available': True, 'method': 'comfy', 'path': shutil.which('comfy')}
    if shutil.which('uvx'):
        return {'available': True, 'method': 'uvx', 'hint': 'Invoke as `uvx --from comfy-cli comfy ...`'}
    return {'available': False, 'hint': 'Install with: pipx install comfy-cli (or `pip install comfy-cli`)'}
...
SMOKE_WORKFLOW = {'3': {'class_type': 'KSampler', 'inputs': {'seed': 1, 'steps': 1, 'cfg': 7.0, 'sampler_name': 'euler', 'scheduler': 'normal', 'denoise': 1.0, 'model': ['4', 0], 'positive': ['6', 0], 'negative': ['7', 0], 'latent_image': ['5', 0]}}, '4': {'class_type': 'CheckpointLoaderSimple', 'inputs': {'ckpt_name': 'REPLACE_ME'}}, '5': {'class_type': 'EmptyLatentImage', 'inputs': {'width': 256, 'height': 256, 'batch_size': 1}}, '6': {'class_type': 'CLIPTextEncode', 'inputs': {'text': 'test', 'clip': ['4', 1]}}, '7': {'class_type': 'CLIPTextEncode', 'inputs': {'text': '', 'clip': ['4', 1]}}, '9': {'class_type': 'SaveImage', 'inputs': {'filename_prefix': 'smoke', 'images': ['3', 0]}}}
...
def smoke_test(host: str, headers: dict, ckpt_name: str | None) -> dict:
    ...
    if '_http_error' in sub:
        return {'ran': True, 'submitted': False, 'http_status': sub['_http_error'], 'body': sub.get('body')}
    ...
    try:
        ...
        pass
    return {'ran': True, 'submitted': True, 'prompt_id': pid, 'cancelled_after_submit': cancelled, 'note': 'Submission accepted; cancelled to avoid running the full pipeline.'}
...
def main(argv: list[str] | None=None) -> int:
    ...
    p.add_argument('--smoke-test', action='store_true', help='Submit a tiny test workflow and verify round-trip')
    p.add_argument('--strict', action='store_true', help='Exit non-zero on any non-pass condition (including warnings)')
    ...
    report = {'verdict': verdict, 'reasons': reasons, 'host': args.host, 'comfy_cli': cli, 'server': server, 'checkpoints': ckpts, 'workflow_check': workflow_check, 'smoke_test': smoke}
    emit_json(report)
    if verdict == 'pass':
        return 0
    if verdict == 'warn':
