...
from _common import DEFAULT_LOCAL_HOST, ENV_API_KEY, log, new_client_id, resolve_api_key, is_cloud_host
...
def parse_binary_frame(data: bytes) -> dict | None:
    ...
    if type_code == BINARY_PREVIEW_IMAGE:
        ...
        return {'kind': 'preview', 'image_type': image_type, 'ext': ext, 'image_bytes': data[8:]}
    if type_code == BINARY_PREVIEW_IMAGE_WITH_METADATA:
        ...
        return {'kind': 'preview_with_metadata', 'metadata': meta, 'image_bytes': data[meta_end:], 'ext': 'png'}
    if type_code == BINARY_TEXT:
        ...
        if len(data) < nid_end:
            return None
        return {'kind': 'text', 'node_id': data[8:nid_end].decode('utf-8', 'replace'), 'text': data[nid_end:].decode('utf-8', 'replace')}
...
def main(argv: list[str] | None=None) -> int:
    ...
    p.add_argument('--prompt-id', default=None, help='Filter to a specific prompt_id (default: all jobs)')
    p.add_argument('--previews', default=None, help='Directory to save in-progress preview frames')
    p.add_argument('--no-color', action='store_true', help='Disable ANSI colour')
    p.add_argument('--timeout', type=float, default=600.0, help='Hard cap on monitor duration (default 600s)')
    args = p.parse_args(argv)
    try:
        import websocket
    except ImportError:
        print(json.dumps({'error': 'websocket-client not installed', 'install': 'pip install websocket-client'}))
        return 1
