...
def show_metadata(path):
    import pymupdf
    doc = pymupdf.open(path)
    print(json.dumps({'pages': len(doc), 'title': doc.metadata.get('title', ''), 'author': doc.metadata.get('author', ''), 'subject': doc.metadata.get('subject', ''), 'creator': doc.metadata.get('creator', ''), 'producer': doc.metadata.get('producer', ''), 'format': doc.metadata.get('format', '')}, indent=2))
