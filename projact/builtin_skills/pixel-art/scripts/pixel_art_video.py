...
def init_fireflies(rng, W, H):
    return [{'x': rng.randint(20, W - 20), 'y': rng.randint(H // 4, H - 20), 'phase': rng.uniform(0, 6.28), 'speed': rng.uniform(0.3, 0.8)} for _ in range(10)]

def draw_fireflies(draw, ff, t, W, H):
    for f in ff:
        if math.sin(t * 1.5 + f['phase']) < 0.15:
            continue
        _px(draw, f['x'] + math.sin(t * f['speed'] + f['phase']) * 3, f['y'] + math.cos(t * f['speed'] * 0.7) * 2, (200, 255, 100), 2)

def init_leaves(rng, W, H):
    return [{'x': rng.randint(0, W), 'y': rng.randint(-H, 0), 'speed': rng.uniform(0.5, 1.5), 'wobble': rng.uniform(0.02, 0.05), 'phase': rng.uniform(0, 6.28), 'color': rng.choice([(180, 120, 50), (160, 100, 40), (200, 140, 60)])} for _ in range(12)]

def draw_leaves(draw, leaves, t, W, H):
    for leaf in leaves:
        _px(draw, leaf['x'] + math.sin(t * leaf['wobble'] + leaf['phase']) * 15, (leaf['y'] + t * leaf['speed'] * 20) % (H + 40) - 20, leaf['color'], 2)

def init_dust_motes(rng, W, H):
    return [{'x': rng.randint(30, W - 30), 'y': rng.randint(30, H - 30), 'phase': rng.uniform(0, 6.28), 'speed': rng.uniform(0.2, 0.5), 'amp': rng.uniform(2, 6)} for _ in range(20)]

def draw_dust_motes(draw, motes, t, W, H):
    for m in motes:
        if math.sin(t * 2.0 + m['phase']) > 0.3:
            _px(draw, m['x'] + math.sin(t * 0.3 + m['phase']) * m['amp'], m['y'] - m['speed'] * t * 15 % H, (255, 210, 100), 1)

def init_sparkles(rng, W, H):
    return [(rng.randint(W // 4, 3 * W // 4), rng.randint(H // 4, 3 * H // 4), rng.uniform(0, 6.28), rng.choice([(180, 200, 255), (255, 220, 150), (200, 180, 255)])) for _ in range(10)]
...
def init_rain(rng, W, H):
    return [{'x': rng.randint(0, W), 'y': rng.randint(0, H), 'speed': rng.uniform(4, 8)} for _ in range(30)]
...
def init_bubbles(rng, W, H):
    return [{'x': rng.randint(20, W - 20), 'y': rng.randint(H, H * 2), 'speed': rng.uniform(0.3, 0.8), 'size': rng.choice([1, 2, 2])} for _ in range(15)]
...
def init_embers(rng, W, H):
    return [{'x': rng.randint(0, W), 'y': rng.randint(0, H), 'speed': rng.uniform(0.3, 0.9), 'phase': rng.uniform(0, 6.28), 'color': rng.choice([(255, 150, 30), (255, 100, 20), (255, 200, 50)])} for _ in range(18)]
...
def init_snowflakes(rng, W, H):
    return [{'x': rng.randint(0, W), 'y': rng.randint(-H, 0), 'speed': rng.uniform(0.3, 0.6), 'wobble': rng.uniform(0.04, 0.09), 'size': rng.choice([2, 2, 3])} for _ in range(40)]
...
def init_neon_pulse(rng, W, H):
    return [(rng.randint(0, W), rng.randint(0, H), rng.uniform(0, 6.28), rng.choice([(255, 0, 200), (0, 255, 255), (255, 50, 150)])) for _ in range(8)]
...
def init_heat_shimmer(rng, W, H):
    return [{'x': rng.randint(0, W), 'y': rng.randint(H // 2, H), 'phase': rng.uniform(0, 6.28)} for _ in range(12)]
...
SCENES = {'night': ['stars', 'fireflies', 'leaves'], 'dusk': ['fireflies', 'sparkles'], 'tavern': ['dust_motes', 'sparkles'], 'indoor': ['dust_motes'], 'urban': ['rain', 'neon_pulse'], 'nature': ['leaves', 'fireflies'], 'magic': ['sparkles', 'fireflies'], 'storm': ['rain', 'lightning'], 'underwater': ['bubbles', 'sparkles'], 'fire': ['embers', 'sparkles'], 'snow': ['snowflakes', 'sparkles'], 'desert': ['heat_shimmer', 'dust_motes']}
_LAYERS = {'stars': (init_stars, draw_stars), 'fireflies': (init_fireflies, draw_fireflies), 'leaves': (init_leaves, draw_leaves), 'dust_motes': (init_dust_motes, draw_dust_motes), 'sparkles': (init_sparkles, draw_sparkles), 'rain': (init_rain, draw_rain), 'lightning': (init_lightning, draw_lightning), 'bubbles': (init_bubbles, draw_bubbles), 'embers': (init_embers, draw_embers), 'snowflakes': (init_snowflakes, draw_snowflakes), 'neon_pulse': (init_neon_pulse, draw_neon_pulse), 'heat_shimmer': (init_heat_shimmer, draw_heat_shimmer)}

def _ensure_ffmpeg():
    if shutil.which('ffmpeg') is None:
        raise RuntimeError('ffmpeg not found on PATH. Install via your package manager or download from https://ffmpeg.org/')

def pixel_art_video(base_image, output_path, scene='night', duration=6, fps=15, seed=None, export_gif=False):
    ...
    with tempfile.TemporaryDirectory(prefix='pixelart_frames_') as frames_dir:
        ...
        subprocess.run(['ffmpeg', '-y', '-loglevel', 'error', '-framerate', str(fps), '-i', os.path.join(frames_dir, 'frame_%04d.png'), '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-crf', '18', output_path], check=True)
        gif_path = None
        if export_gif:
            gif_path = output_path.rsplit('.', 1)[0] + '.gif'
            subprocess.run(['ffmpeg', '-y', '-loglevel', 'error', '-framerate', str(fps), '-i', os.path.join(frames_dir, 'frame_%04d.png'), '-vf', 'scale=320:-1:flags=neighbor,split[s0][s1];[s0]palettegen[p];[s1][p]paletteuse', '-loop', '0', gif_path], check=True)
...
def main():
    ...
    args = p.parse_args()
    mp4, gif = pixel_art_video(args.base_image, args.output, scene=args.scene, duration=args.duration, fps=args.fps, seed=args.seed, export_gif=args.gif)
    print(f'Wrote {mp4}')
    if gif:
        print(f'Wrote {gif}')
