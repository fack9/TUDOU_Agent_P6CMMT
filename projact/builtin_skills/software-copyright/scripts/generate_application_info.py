...
FIELD_ORDER = ['软件全称', '版本号', '著作权人', '开发完成日期', '首次发表日期', '开发的硬件环境', '运行的硬件环境', '开发该软件的操作系统', '软件开发环境 / 开发工具', '该软件的运行平台 / 操作系统', '软件运行支撑环境 / 支持软件', '编程语言', '源程序量', '开发目的', '面向领域 / 行业', '软件的主要功能', '技术特点', '软件的技术特点选项', '页数', '软件分类']
...
def humanize_feature(name: str) -> str:
    value = re.sub('([a-z])([A-Z])', '\\1 \\2', name)
    value = value.replace('-', ' ').replace('_', ' ').strip()
    key = value.lower().replace(' ', '')
    mapping = {'login': '软件登录', 'register': '用户注册', 'auth': '用户认证', 'home': '首页', 'dashboard': '数据看板', 'project': '项目管理', 'projects': '项目管理', 'projectsettings': '项目设置', 'projectssettings': '项目设置', 'settings': '系统设置', 'asset': '资源管理', 'assets': '资源管理', 'assethub': '资源中心', 'billing': '费用管理', 'agentstatusbar': '智能体状态展示', 'messagebubble': '消息展示', 'chatpanel': '对话面板', 'chatinput': '对话输入', 'assetpanel': '资源面板'}
    return mapping.get(key, value.title() if re.search('[A-Za-z]', value) else value)

def build_fields(analysis: dict[str, Any], manifest: dict[str, Any], software_name: str, version: str, answers: dict[str, str], business: dict[str, Any] | None=None) -> dict[str, str]:
    ...
    defaults = {'软件全称': software_name_hint, '版本号': version_hint, '著作权人': '待用户确认', '开发完成日期': '待用户确认', '首次发表日期': '待用户确认', '开发的硬件环境': hardware_hint, '运行的硬件环境': hardware_hint, '开发该软件的操作系统': dev_os_hint, '软件开发环境 / 开发工具': infer_ide_name(project), '该软件的运行平台 / 操作系统': infer_runtime_os(analysis), '软件运行支撑环境 / 支持软件': infer_runtime_support(analysis, project), '编程语言': language, '源程序量': str(analysis.get('source', {}).get('line_count') or manifest.get('selected_source_line_count') or '待用户确认'), '开发目的': business.get('application_purpose') if business else f'建设{software_name}，为用户提供稳定、便捷的信息化操作能力，提升相关业务处理效率。', '面向领域 / 行业': business.get('industry') if business else '待用户确认', '软件的主要功能': business.get('main_functions') if business else summarize_features(analysis, software_name), '技术特点': business.get('technical_characteristics') if business else f'系统采用{framework_text}构建前端界面，结合模块化组件、路由组织、接口封装和状态管理实现业务功能，具备较好的可维护性和扩展性。', '软件的技术特点选项': business.get('software_technical_option') if business else '应用软件', '页数': str(manifest.get('total_pages') or '待用户确认'), '软件分类': business.get('software_category') if business else '应用软件'}
...
def version_confirmation_hint(analysis: dict[str, Any], requested_version: str) -> str:
    ...
    if project_version and version_less_than_1(project_version):
        return f'待用户确认（项目版本号为 {project_version}，软著首次提交通常建议从 V1.0 开始；请确认填写 V1.0 还是 {project_version}）'
    if not project_version and version_less_than_1(requested):
        return f'待用户确认（当前建议版本号为 {requested}，软著首次提交通常建议从 V1.0 开始；请确认填写 V1.0 还是 {requested}）'
...
def detect_package_manager(project: Path, package_path: str) -> str:
    ...
    checks = [('pnpm-lock.yaml', 'pnpm'), ('yarn.lock', 'Yarn'), ('bun.lock', 'Bun'), ('bun.lockb', 'Bun'), ('package-lock.json', 'npm')]
...
def write_application_md(path: Path, fields: dict[str, str], analysis: dict[str, Any], manifest: dict[str, Any], business: dict[str, Any] | None=None) -> None:
    ...
    lines.extend(['', '## 环境字段填写口径', '', '- 软件全称：必须由用户确认；最终正式资料文件名、代码页眉和操作手册中的软件名称均以本字段为准。', '- 软件开发环境 / 开发工具：填写 IDE 或编辑器名称，例如 Visual Studio Code、WebStorm、IntelliJ IDEA、Cursor。', '- 版本号：必须由用户确认；如果项目版本小于 V1.0，软著首次提交通常建议使用 V1.0，也可按实际项目版本填写，最终以前面“版本号”字段为准。', '- 开发该软件的操作系统：填写实际开发电脑的操作系统版本，例如 macOS 14、macOS 15、Windows 10、Windows 11。', '- 该软件的运行平台 / 操作系统：填写软件客户端或服务运行所在的操作系统版本，例如 Windows 10/11 或 macOS 13及以上版本。', '- 软件运行支撑环境 / 支持软件：填写项目运行所需的软件环境，例如 Node.js、Python、Docker、数据库、浏览器、中间件或外部服务。', '- 开发的硬件环境和运行的硬件环境：可使用当前检测到的电脑配置作为建议值，也可按实际开发、部署或审核口径调整。', '', '## 项目分析摘要', '', f'- 项目目录：{analysis.get('project_root', '')}', f'- 框架：{'、'.join(analysis.get('frameworks') or []) or '未识别'}', f'- 源码文件数：{analysis.get('source', {}).get('file_count', 0)}', f'- 代码材料页数：{manifest.get('total_pages', 0)}', f'- 代码输出模式：{manifest.get('mode', '')}', f'- 业务理解：{('已读取 草稿/业务理解.json' if business else '未提供，使用项目分析兜底')}', '', '## 待确认字段', ''])
    if pending:
        lines.extend((f'- {field}' for field in pending))
    else:
        lines.append('- 无')
    lines.extend(['', '```text', 'STOP_FOR_USER', 'NEXT_ACTION: 请补全并确认申请表字段；确认后运行 confirm_stage.py --stage application-fields。', '```'])
    path.write_text('\n'.join(lines) + '\n', encoding='utf-8')
