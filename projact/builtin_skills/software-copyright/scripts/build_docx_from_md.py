...
def parse_code_pages(md_path: Path) -> list[tuple[int, list[str]]]:
    ...
    for raw in md_path.read_text(encoding='utf-8').splitlines():
        page_match = re.match('^##\\s+第\\s*(\\d+)\\s*页', raw.strip())
        if page_match:
...
def paragraph_xml(text: str, font: str='SimSun', size_half_points: int=21, align: str | None=None, line_twips: int=240) -> str:
    ...
    return f'<w:p><w:pPr>{align_xml}<w:spacing w:after="0" w:line="{line_twips}" w:lineRule="exact"/></w:pPr><w:r><w:rPr><w:rFonts w:ascii="{font}" w:hAnsi="{font}" w:eastAsia="{font}"/><w:color w:val="{BLACK_RGB}"/><w:sz w:val="{size_half_points}"/><w:szCs w:val="{size_half_points}"/></w:rPr><w:t xml:space="preserve">{escaped}</w:t></w:r></w:p>'

def page_break_xml() -> str:
    return '<w:p><w:r><w:br w:type="page"/></w:r></w:p>'

def page_field_runs_xml() -> str:
    return f'<w:r><w:rPr><w:rFonts w:ascii="SimSun" w:hAnsi="SimSun" w:eastAsia="SimSun"/><w:color w:val="{BLACK_RGB}"/><w:sz w:val="16"/><w:szCs w:val="16"/></w:rPr><w:fldChar w:fldCharType="begin"/></w:r><w:r><w:rPr><w:rFonts w:ascii="SimSun" w:hAnsi="SimSun" w:eastAsia="SimSun"/><w:color w:val="{BLACK_RGB}"/><w:sz w:val="16"/><w:szCs w:val="16"/></w:rPr><w:instrText xml:space="preserve"> PAGE </w:instrText></w:r><w:r><w:rPr><w:rFonts w:ascii="SimSun" w:hAnsi="SimSun" w:eastAsia="SimSun"/><w:color w:val="{BLACK_RGB}"/><w:sz w:val="16"/><w:szCs w:val="16"/></w:rPr><w:fldChar w:fldCharType="separate"/></w:r><w:r><w:rPr><w:rFonts w:ascii="SimSun" w:hAnsi="SimSun" w:eastAsia="SimSun"/><w:color w:val="{BLACK_RGB}"/><w:sz w:val="16"/><w:szCs w:val="16"/></w:rPr><w:t>1</w:t></w:r><w:r><w:rPr><w:rFonts w:ascii="SimSun" w:hAnsi="SimSun" w:eastAsia="SimSun"/><w:color w:val="{BLACK_RGB}"/><w:sz w:val="16"/><w:szCs w:val="16"/></w:rPr><w:fldChar w:fldCharType="end"/></w:r>'
...
def minimal_docx(out_path: Path, body_xml: str, header_text: str | None=None) -> None:
    ...
    header_rel = '<Relationship Id="rIdHeader1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/header" Target="header1.xml"/>' if header_text else ''
...
def force_black_xml(xml: str) -> str:
    xml = re.sub('<w:hyperlink\\b[^>]*>', '', xml)
    xml = xml.replace('</w:hyperlink>', '')
    xml = re.sub('<w:color\\b[^>]*/>', f'<w:color w:val="{BLACK_RGB}"/>', xml)
    ...
    xml = re.sub('<w:rPr\\b[^>]*>.*?</w:rPr>', ensure_rpr_color, xml, flags=re.S)
    xml = re.sub('<w:r>(?!<w:rPr>)', f'<w:r><w:rPr><w:color w:val="{BLACK_RGB}"/></w:rPr>', xml)
    return xml

def normalize_docx_text_color(docx_path: Path) -> None:
    tmp_path = docx_path.with_suffix(docx_path.suffix + '.tmp')
    color_xml_parts = ('word/document.xml', 'word/styles.xml', 'word/numbering.xml', 'word/header', 'word/footer')
        ...
        for item in src.infolist():
            ...
            if item.filename.endswith('.xml') and item.filename.startswith(color_xml_parts):
                ...
                if 'hyperlink' in text:
                    text = re.sub('\\s*<Relationship\\b[^>]*Type="[^"]*/hyperlink"[^>]*/>', '', text)
...
def build_manual_docx_python(md_path: Path, out_path: Path, base_dir: Path) -> None:
    ...
    def flush_table() -> None:
        nonlocal table_buf
        if table_buf:
            data = [row for row in table_buf if not all((re.fullmatch(':?-{3,}:?', cell) for cell in row))]
    ...
    for line in lines:
        ...
        if not stripped:
            continue
        image_match = re.search('!\\[[^\\]]*\\]\\(([^)]+)\\)', stripped)
        if image_match:
            add_image(document, (base_dir / image_match.group(1)).resolve())
            continue
        heading = re.match('^(#{1,4})\\s+(.+)$', stripped)
        if heading:
        ...
        if re.match('^[-*+]\\s+', stripped):
            ...
            run = p.add_run(re.sub('^[-*+]\\s+', '', stripped))
            set_run_font(run, 'SimSun', 10.5)
            continue
        if re.match('^\\d+\\.\\s+', stripped):
            p = document.add_paragraph(style='List Number')
            run = p.add_run(re.sub('^\\d+\\.\\s+', '', stripped))
            set_run_font(run, 'SimSun', 10.5)
            continue
        p = document.add_paragraph()
...
def build_with_pandoc(md_path: Path, out_path: Path, code_mode: bool=False) -> None:
    ...
    text = original_text
    text = re.sub('```text\\s*\\nSTOP_FOR_USER\\n.*?```', '', text, flags=re.S)
    text = re.sub('<!--[^>]*截图[^>]*-->', '【截图预留：请在此处插入当前功能页面或操作结果截图。】', text)
    text = strip_markdown_links(text)
    if code_mode:
        text = re.sub('(?=^##\\s+第\\s*\\d+\\s*页)', '\\n\\\\newpage\\n', text, flags=re.M)
...
def build_all(workdir: Path, software_name: str, version: str, skip_preview: bool) -> dict[str, Any]:
    ...
    code_specs = [('代码-前30页.md', f'{safe_name}-代码(前30页).docx'), ('代码-后30页.md', f'{safe_name}-代码(后30页).docx'), ('代码-全部.md', f'{safe_name}-代码(全部).docx')]
