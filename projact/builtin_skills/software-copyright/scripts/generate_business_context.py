...
def strip_md(text: str) -> str:
    text = re.sub('`{3}.*?`{3}', ' ', text, flags=re.S)
    text = re.sub('\\[(.*?)\\]\\(.*?\\)', '\\1', text)
    text = re.sub('[>#*_`|]', ' ', text)
...
def skip_doc(path: Path, project: Path) -> bool:
    ...
    skip_parts = ('node_modules', '.git/', 'dist/', 'build/', '.next/', 'coverage/', '软件著作权申请资料')
...
def collect_documents(project: Path) -> list[dict[str, Any]]:
    ...
    for path in iter_project_files(project, DOC_EXTS):
        ...
        if not text.strip():
            continue
        docs.append({'path': rel(path, project), 'size': path.stat().st_size, 'headings': extract_headings(text), 'opening': extract_opening(text)})
...
def collect_code_evidence(analysis: dict[str, Any]) -> dict[str, Any]:
    ...
    return {'project_name': analysis.get('project_name'), 'software_name_candidate': analysis.get('software_name_candidate'), 'frameworks': analysis.get('frameworks') or [], 'language': analysis.get('language'), 'routes': analysis.get('routes') or [], 'feature_name_candidates': analysis.get('feature_candidates') or [], 'entry_files': categorized.get('entry') or [], 'page_files': categorized.get('page') or [], 'component_files': categorized.get('component') or [], 'api_files': categorized.get('api') or [], 'run_command_candidates': analysis.get('run_command_candidates') or [], 'package': analysis.get('package') or {}}

def build_evidence(project: Path, analysis: dict[str, Any], software_name: str, web_notes: str) -> dict[str, Any]:
    return {'software_name': software_name, 'project_root': str(project.resolve()), 'instruction': '本文件只收集证据，不决定行业、功能或手册结构。请由模型阅读这些证据以及必要的项目源码后，另行编写业务理解模型稿。', 'documents': collect_documents(project), 'code_evidence': collect_code_evidence(analysis), 'external_research_notes': web_notes}

def write_evidence_md(path: Path, evidence: dict[str, Any]) -> None:
    lines = ['# 业务理解证据', '', f'- 软件名称：{evidence['software_name']}', f'- 项目目录：`{evidence['project_root']}`', '', '本文件只列出可供模型研判的项目证据，不代表最终申报口径。', '模型需要自行判断应阅读哪些文档、抽取哪些功能、采用什么操作手册结构。', '', '## 代码与页面证据', '']
    ...
    for doc in evidence['documents']:
        lines.extend([f'### {doc['path']}', '', f'- 大小：{doc['size']} bytes', f'- 标题线索：{('；'.join(doc['headings']) if doc['headings'] else '无')}', '', doc['opening'], ''])
...
def write_model_template(path: Path, evidence: dict[str, Any]) -> None:
    template = {'software_name': evidence['software_name'], 'product_positioning': '', 'industry': '', 'target_users': [], 'core_value': '', 'business_features': [], 'business_feature_details': {}, 'operation_flow': [], 'application_purpose': '', 'main_functions': '', 'technical_characteristics': '', 'software_technical_option': '应用软件', 'software_category': '应用软件', 'manual_sections': [{'title': '模型自行命名章节', 'intent': '说明该章节为什么适合当前项目。', 'paragraphs': [], 'include_feature_overview': False, 'include_operation_modules': False, 'include_operation_flow': False}], 'model_review_notes': ['不要照抄范本结构；按当前项目业务组织章节。', '不要用关键词表决定行业和功能；必须能从项目证据或用户补充中解释来源。']}
...
def normalize_model_context(model: dict[str, Any], evidence: dict[str, Any], web_notes: str) -> dict[str, Any]:
    ...
    context = {'software_name': evidence['software_name'], 'business_understanding_required': True, 'source_documents': [{'path': doc['path'], 'size': doc['size']} for doc in evidence['documents']], 'project_evidence_file': '业务理解证据.md', 'product_positioning': required_text(model, 'product_positioning'), 'industry': required_text(model, 'industry'), 'target_users': required_list(model.get('target_users'), 'target_users'), 'core_value': required_text(model, 'core_value'), 'business_features': features, 'business_feature_details': {feature: str(details.get(feature)).strip() for feature in features}, 'operation_flow': required_list(model.get('operation_flow'), 'operation_flow'), 'application_purpose': required_text(model, 'application_purpose'), 'main_functions': required_text(model, 'main_functions'), 'technical_characteristics': required_text(model, 'technical_characteristics'), 'software_technical_option': str(model.get('software_technical_option') or '应用软件'), 'software_category': str(model.get('software_category') or '应用软件'), 'manual_sections': sections, 'model_authored': True, 'external_research_notes': web_notes, 'confirmation_required': True, 'user_confirmed': False, 'confirmation_stage': 'business', 'next_action': '请确认 草稿/业务理解.md 中的软件用途、行业、目标用户、核心功能、手册结构和申请口径；确认后运行 confirm_stage.py --stage business。', 'review_notes': ['请确认模型判断的行业领域、目标用户和主要功能是否符合实际申报口径。', '请确认操作手册结构是否适合当前项目，而不是套用范本文档。']}
    return context

def write_context_md(path: Path, context: dict[str, Any]) -> None:
    lines = ['# 业务理解', '', f'- 软件名称：{context['software_name']}', f'- 产品定位：{context['product_positioning']}', f'- 面向领域 / 行业：{context['industry']}', f'- 核心价值：{context['core_value']}', f'- 证据文件：`{context['project_evidence_file']}`', '', '## 目标用户', '']
    ...
    lines.extend(['', '## 申请表建议口径', '', f'- 开发目的：{context['application_purpose']}', f'- 软件的主要功能：{context['main_functions']}', f'- 技术特点：{context['technical_characteristics']}', f'- 软件的技术特点选项：{context['software_technical_option']}', f'- 软件分类：{context['software_category']}', '', '## 证据来源', ''])
    lines.extend((f'- `{item['path']}`' for item in context['source_documents']))
    lines.extend(['', '## 待确认', ''])
    lines.extend((f'- {item}' for item in context['review_notes']))
    lines.extend(['', '```text', 'STOP_FOR_USER', f'NEXT_ACTION: {context['next_action']}', '```'])
    path.write_text('\n'.join(lines) + '\n', encoding='utf-8')
