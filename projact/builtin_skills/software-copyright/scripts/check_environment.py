...
def check_environment(skill_dir: Path) -> dict[str, Any]:
    ...
    next_action = '请选择：1) 安装完整 DOCX 环境；2) 使用基础 DOCX 兜底继续。回复选择后再进入项目分析。' if requires_user_input else '完整 DOCX 环境可用，可以进入项目分析。'
    return {'output_directory': '当前目录/软件著作权申请资料', 'capabilities': {'markdown_drafts': True, 'application_txt': True, 'basic_docx': python_docx or True, 'python_docx': python_docx, 'pandoc_preview': pandoc_ok, 'docx_openxml_full': docx_ready, 'dotnet_sdk': dotnet_ok}, 'versions': {'pandoc': pandoc_version, 'dotnet': dotnet_version}, 'final_docx_mode': final_docx_mode, 'recommendation': '完整 DOCX OpenXML 环境已就绪，建议使用完整 Word 生成和校验流程。' if docx_ready else '完整 DOCX OpenXML 环境未就绪。可以继续使用兜底 DOCX 生成；如需更规范的 Word 结构和校验，请先安装 .NET SDK 并运行 vendor/docx-toolkit/scripts/setup.sh。', 'install_prompt': '是否安装完整 DOCX 环境？安装后文档生成和校验更规范；不安装也可以继续生成 Markdown、TXT 和基础 DOCX。' if not docx_ready else '无需安装，完整环境可用。', 'requires_user_input': requires_user_input, 'confirmation_stage': 'environment' if requires_user_input else None, 'next_action': next_action, 'docx_env_output': docx_output}
