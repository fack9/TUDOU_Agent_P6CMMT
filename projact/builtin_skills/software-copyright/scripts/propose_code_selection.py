...
def build_candidates(project: Path) -> list[dict[str, Any]]:
    ...
    for path in files:
        try:
            ...
            line_count = 0
        priority, _ = category_weight(path, project)
        candidates.append({'path': rel(path, project), 'selected': False, 'start_line': 1, 'end_line': None, 'line_count': line_count, 'priority': priority, 'selection_tier': 'frontend' if path.suffix.lower() in FRONTEND_EXTS else 'supplement', 'evidence': evidence_for(path, project), 'model_reason': ''})
    return candidates
...
def selection_stats(candidates: list[dict[str, Any]]) -> dict[str, int]:
    ...
    return {'selected_count': len(selected_items), 'selected_lines': sum((selected_line_estimate(item) for item in selected_items))}
...
def write_selection_md(path: Path, data: dict[str, Any]) -> None:
    lines = ['# 代码文件候选清单', '', '请先确认要抽取哪些源码文件，再运行代码材料抽取。', '', '本清单只列出候选源码证据，不默认决定抽取文件。', '模型需要先理解项目业务、页面入口和源码职责，再填写 `selected/start_line/end_line/model_reason`。', f'当前已选约 {data['estimated_selected_pages']} 页，全部候选源码约 {data['estimated_all_candidate_pages']} 页。', '', '```text', 'STOP_FOR_USER', 'NEXT_ACTION: 请由模型先填写 草稿/代码文件选择.json 的抽取选择和选择理由，再让用户确认；确认后运行 confirm_stage.py --stage code-selection。', '```', '', '确认方式：', '', '1. 模型根据项目业务和代码入口选择最能体现软件功能的文件。', '2. 把需要抽取的文件设为 `selected: true`，并填写 `model_reason`。', '3. 如只想抽取某个文件中间部分，可填写 `start_line` 和 `end_line`。', '4. 用户确认模型选择后，再记录 `code-selection` 门禁。', '', '## 默认选中文件', '', '| 文件 | 行数 | 抽取范围 | 模型选择理由 |', '| --- | ---: | --- | --- |']
