...
def category_weight(path: Path, project: Path) -> tuple[int, str]:
    ...
    priority = 80
    if name in {'main.ts', 'main.js', 'main.tsx', 'main.jsx', 'app.vue', 'app.tsx'} or r in {'src/app/page.tsx', 'src/app/layout.tsx', 'app/page.tsx', 'app/layout.tsx'} or r.endswith('/src/app/page.tsx') or r.endswith('/src/app/layout.tsx'):
        priority = 0
...
def should_skip_file(path: Path) -> bool:
    ...
    try:
        ...
        return True
    if size <= 0 or size > 800000:
        return True
    try:
        sample = read_text(path, limit=20000)
    except Exception:
        return True
...
def load_selected_files(project: Path, selection_path: Path | None) -> list[dict[str, Any]]:
    ...
    if isinstance(data, dict) and data.get('selection_required') and (not data.get('user_confirmed')):
        raise SystemExit('STOP_FOR_USER\nNEXT_ACTION: 代码文件选择尚未确认。请先确认或修改 草稿/代码文件选择.json，再运行 `python3 scripts/confirm_stage.py --workdir 软件著作权申请资料 --stage code-selection --note "<用户确认内容>"`。')
    ...
    for item in items:
        ...
        if not path_value:
            continue
        selected.append({'path': str(path_value), 'start_line': item.get('start_line') or 1, 'end_line': item.get('end_line'), 'selected': True})
    return selected
...
def collect_code_lines(project: Path, selection_path: Path | None) -> tuple[list[str], list[dict[str, Any]]]:
    ...
    for item in selected_items:
        ...
        manifest_files.append({'path': rel(path, project), 'source_line_count': len(source_lines), 'selected_line_start': start_line, 'selected_line_end': end_line, 'selected_line_count': len(selected_lines), 'material_line_start': start, 'material_line_end': end})
...
def write_manifest_md(path: Path, manifest: dict[str, Any]) -> None:
    lines = ['# 代码提取清单', '', f'- 软件名称：{manifest['software_name']}', f'- 版本号：{manifest['version']}', f'- 项目目录：{manifest['project_root']}', f'- 源码文件数：{manifest['file_count']}', f'- 材料代码行数：{manifest['material_line_count']}', f'- 每页行数：{manifest['lines_per_page']}', f'- 总页数：{manifest['total_pages']}', f'- 目标页数：{manifest['target_pages']}', f'- 候选源码可生成页数：{manifest['available_candidate_pages']}', f'- 补充状态：{manifest['supplement_status']}', f'- 输出模式：{manifest['mode']}', '', '## 文件来源', '', '| 文件 | 源码行数 | 抽取源码范围 | 抽取行数 | 材料行范围 |', '| --- | ---: | --- | ---: | --- |']
    for item in manifest['files']:
        lines.append(f'| `{item['path']}` | {item['source_line_count']} | {item['selected_line_start']}-{item['selected_line_end']} | {item['selected_line_count']} | {item['material_line_start']}-{item['material_line_end']} |')
    path.write_text('\n'.join(lines) + '\n', encoding='utf-8')
...
def extract(project: Path, out_dir: Path, software_name: str, version: str, lines_per_page: int, selection_path: Path | None) -> dict[str, Any]:
    ...
    if total_pages < SPLIT_THRESHOLD_PAGES and available_pages >= SPLIT_THRESHOLD_PAGES and (unselected_count > 0):
        raise SystemExit(f'STOP_FOR_USER\nNEXT_ACTION: 当前已选代码只有 {total_pages} 页，但候选源码足够补齐到 {SPLIT_THRESHOLD_PAGES} 页。请在 草稿/代码文件选择.json 中继续选择补充文件，重新记录 code-selection 门禁后再抽取。')
    outputs: list[str] = []
