...
DEPENDENCY_FRAMEWORKS = {'vue': 'Vue', '@vue/runtime-core': 'Vue', 'react': 'React', 'next': 'Next.js', 'nuxt': 'Nuxt', 'svelte': 'Svelte', 'astro': 'Astro', '@angular/core': 'Angular', 'vite': 'Vite', 'uni-app': 'UniApp', '@dcloudio/uni-app': 'UniApp', 'electron': 'Electron', '@tauri-apps/api': 'Tauri'}
ENTRY_NAMES = {'main.ts', 'main.js', 'main.tsx', 'main.jsx', 'index.tsx', 'index.jsx', 'app.vue', 'App.vue', 'app.tsx'}

def load_package(project: Path) -> tuple[dict[str, Any] | None, Path | None]:
    candidates = [project / 'package.json', project / 'frontend/package.json', project / 'client/package.json', project / 'web/package.json', project / 'app/package.json']
...
def extract_route_paths(path: Path) -> list[str]:
    try:
        ...
        return []
    patterns = ['path\\s*:\\s*[\'\\"]([^\'\\"]+)[\'\\"]', '<Route[^>]+path=[\'\\"]([^\'\\"]+)[\'\\"]', 'href=[\'\\"](/[^\'\\"]*)[\'\\"]']
    routes: list[str] = []
...
def analyze(project: Path) -> dict[str, Any]:
    ...
    source_lines = 0
    categorized: dict[str, list[str]] = {'entry': [], 'route': [], 'page': [], 'component': [], 'api': [], 'state': [], 'utility': [], 'style': [], 'source': []}
    ...
    return {'project_root': str(project), 'project_name': project.name, 'software_name_candidate': normalize_title(package_name or project.name), 'package': {'name': package_name, 'path': rel(package_path, project) if package_path else '', 'version': str(package.get('version') or 'V1.0') if package else 'V1.0', 'scripts': scripts, 'dependency_names': sorted(dependencies)}, 'frameworks': frameworks, 'language': language, 'source': {'file_count': len(source_files), 'line_count': source_lines, 'extension_counts': dict(sorted(extension_counts.items())), 'category_counts': dict(sorted(class_counts.items())), 'categorized_files': {k: v[:80] for k, v in categorized.items() if v}}, 'routes': route_paths[:80], 'readme_excerpt': summarize_readme(project), 'run_command_candidates': infer_run_commands(scripts), 'feature_candidates': infer_features(categorized, route_paths)}
...
def check_environment_gate(out: Path) -> None:
    ...
    if not confirmed:
        raise SystemExit('STOP_FOR_USER\nNEXT_ACTION: 完整 DOCX 环境未确认。请先让用户选择安装完整环境或使用基础 DOCX 兜底继续，然后运行 `python3 scripts/confirm_stage.py --workdir 软件著作权申请资料 --stage environment --note "<用户选择>"`。')
...
def infer_features(categorized: dict[str, list[str]], routes: list[str]) -> list[str]:
    stop = {'index', 'main', 'app', 'layout', 'page', 'globals', 'providers', 'loading', 'error', 'not-found', 'template', 'default', 'button', 'input', 'label', 'avatar', 'card', 'textarea', 'scroll area'}
    names: list[str] = []
    for route in routes:
    ...
    unique: list[str] = []
    for name in names:
        normalized = re.sub('\\s+', ' ', name).strip()
