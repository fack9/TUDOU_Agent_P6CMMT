...
def gmail_search(args):
    if _gws_binary():
        results = _run_gws(['gmail', 'users', 'messages', 'list'], params={'userId': 'me', 'q': args.query, 'maxResults': args.max})
        messages = results.get('messages', [])
        output = []
        for msg_meta in messages:
            msg = _run_gws(['gmail', 'users', 'messages', 'get'], params={'userId': 'me', 'id': msg_meta['id'], 'format': 'metadata', 'metadataHeaders': ['From', 'To', 'Subject', 'Date']})
    ...
    output = []
    for msg_meta in messages:
        msg = service.users().messages().get(userId='me', id=msg_meta['id'], format='metadata', metadataHeaders=['From', 'To', 'Subject', 'Date']).execute()
        headers = _headers_dict(msg)
        output.append({'id': msg['id'], 'threadId': msg['threadId'], 'from': headers.get('From', ''), 'to': headers.get('To', ''), 'subject': headers.get('Subject', ''), 'date': headers.get('Date', ''), 'snippet': msg.get('snippet', ''), 'labels': msg.get('labelIds', [])})
    print(json.dumps(output, indent=2, ensure_ascii=False))
...
def gmail_get(args):
    if _gws_binary():
        msg = _run_gws(['gmail', 'users', 'messages', 'get'], params={'userId': 'me', 'id': args.message_id, 'format': 'full'})
        headers = _headers_dict(msg)
        result = {'id': msg['id'], 'threadId': msg['threadId'], 'from': headers.get('From', ''), 'to': headers.get('To', ''), 'subject': headers.get('Subject', ''), 'date': headers.get('Date', ''), 'labels': msg.get('labelIds', []), 'body': _extract_message_body(msg)}
...
def gmail_send(args):
    if _gws_binary():
        ...
        result = _run_gws(['gmail', 'users', 'messages', 'send'], params={'userId': 'me'}, body=body)
...
def gmail_reply(args):
    if _gws_binary():
        original = _run_gws(['gmail', 'users', 'messages', 'get'], params={'userId': 'me', 'id': args.message_id, 'format': 'metadata', 'metadataHeaders': ['From', 'Subject', 'Message-ID']})
        ...
        raw = base64.urlsafe_b64encode(message.as_bytes()).decode()
        result = _run_gws(['gmail', 'users', 'messages', 'send'], params={'userId': 'me'}, body={'raw': raw, 'threadId': original['threadId']})
        print(json.dumps({'status': 'sent', 'id': result['id'], 'threadId': result.get('threadId', '')}, indent=2))
        return
    service = build_service('gmail', 'v1')
    original = service.users().messages().get(userId='me', id=args.message_id, format='metadata', metadataHeaders=['From', 'Subject', 'Message-ID']).execute()
    headers = _headers_dict(original)
    subject = headers.get('Subject', '')
    if not subject.startswith('Re:'):
        subject = f'Re: {subject}'
...
def gmail_modify(args):
    ...
    if _gws_binary():
        result = _run_gws(['gmail', 'users', 'messages', 'modify'], params={'userId': 'me', 'id': args.message_id}, body=body)
...
def calendar_list(args):
    ...
    if _gws_binary():
        results = _run_gws(['calendar', 'events', 'list'], params={'calendarId': args.calendar, 'timeMin': time_min, 'timeMax': time_max, 'maxResults': args.max, 'singleEvents': True, 'orderBy': 'startTime'})
        events = []
    ...
    results = service.events().list(calendarId=args.calendar, timeMin=time_min, timeMax=time_max, maxResults=args.max, singleEvents=True, orderBy='startTime').execute()
    events = []
    for e in results.get('items', []):
        events.append({'id': e['id'], 'summary': e.get('summary', '(no title)'), 'start': e.get('start', {}).get('dateTime', e.get('start', {}).get('date', '')), 'end': e.get('end', {}).get('dateTime', e.get('end', {}).get('date', '')), 'location': e.get('location', ''), 'description': e.get('description', ''), 'status': e.get('status', ''), 'htmlLink': e.get('htmlLink', '')})
    print(json.dumps(events, indent=2, ensure_ascii=False))
...
def calendar_create(args):
    ...
    if _gws_binary():
        result = _run_gws(['calendar', 'events', 'insert'], params={'calendarId': args.calendar}, body=event)
        print(json.dumps({'status': 'created', 'id': result['id'], 'summary': result.get('summary', ''), 'htmlLink': result.get('htmlLink', '')}, indent=2))
        return
    service = build_service('calendar', 'v3')
...
def drive_search(args):
    ...
    if _gws_binary():
        results = _run_gws(['drive', 'files', 'list'], params={'q': query, 'pageSize': args.max, 'fields': 'files(id, name, mimeType, modifiedTime, webViewLink)'})
        print(json.dumps(results.get('files', []), indent=2, ensure_ascii=False))
        return
    service = build_service('drive', 'v3')
    results = service.files().list(q=query, pageSize=args.max, fields='files(id, name, mimeType, modifiedTime, webViewLink)').execute()
    files = results.get('files', [])
    print(json.dumps(files, indent=2, ensure_ascii=False))

def contacts_list(args):
    if _gws_binary():
        results = _run_gws(['people', 'people', 'connections', 'list'], params={'resourceName': 'people/me', 'pageSize': args.max, 'personFields': 'names,emailAddresses,phoneNumbers'})
        contacts = []
        for person in results.get('connections', []):
            names = person.get('names', [{}])
            emails = person.get('emailAddresses', [])
            phones = person.get('phoneNumbers', [])
            contacts.append({'name': names[0].get('displayName', '') if names else '', 'emails': [e.get('value', '') for e in emails], 'phones': [p.get('value', '') for p in phones]})
        print(json.dumps(contacts, indent=2, ensure_ascii=False))
        return
    service = build_service('people', 'v1')
    results = service.people().connections().list(resourceName='people/me', pageSize=args.max, personFields='names,emailAddresses,phoneNumbers').execute()
    contacts = []
    for person in results.get('connections', []):
        names = person.get('names', [{}])
        emails = person.get('emailAddresses', [])
...
def sheets_get(args):
    if _gws_binary():
        result = _run_gws(['sheets', 'spreadsheets', 'values', 'get'], params={'spreadsheetId': args.sheet_id, 'range': args.range})
...
def sheets_update(args):
    ...
    body = {'values': values}
    if _gws_binary():
        result = _run_gws(['sheets', 'spreadsheets', 'values', 'update'], params={'spreadsheetId': args.sheet_id, 'range': args.range, 'valueInputOption': 'USER_ENTERED'}, body=body)
        print(json.dumps({'updatedCells': result.get('updatedCells', 0), 'updatedRange': result.get('updatedRange', '')}, indent=2))
        return
    service = build_service('sheets', 'v4')
    result = service.spreadsheets().values().update(spreadsheetId=args.sheet_id, range=args.range, valueInputOption='USER_ENTERED', body=body).execute()
    print(json.dumps({'updatedCells': result.get('updatedCells', 0), 'updatedRange': result.get('updatedRange', '')}, indent=2))
...
def sheets_append(args):
    ...
    body = {'values': values}
    if _gws_binary():
        result = _run_gws(['sheets', 'spreadsheets', 'values', 'append'], params={'spreadsheetId': args.sheet_id, 'range': args.range, 'valueInputOption': 'USER_ENTERED', 'insertDataOption': 'INSERT_ROWS'}, body=body)
        print(json.dumps({'updatedCells': result.get('updates', {}).get('updatedCells', 0)}, indent=2))
        return
    service = build_service('sheets', 'v4')
    result = service.spreadsheets().values().append(spreadsheetId=args.sheet_id, range=args.range, valueInputOption='USER_ENTERED', insertDataOption='INSERT_ROWS', body=body).execute()
    print(json.dumps({'updatedCells': result.get('updates', {}).get('updatedCells', 0)}, indent=2))

def docs_get(args):
    if _gws_binary():
        doc = _run_gws(['docs', 'documents', 'get'], params={'documentId': args.doc_id})
        result = {'title': doc.get('title', ''), 'documentId': doc.get('documentId', ''), 'body': _extract_doc_text(doc)}
        print(json.dumps(result, indent=2, ensure_ascii=False))
        return
